...
class InstagramAPI{
    ...
    func likeOp(type: Bool, media_id: String, username: String, user_id: Int) -> Void {
        ...
        if type{
            ...
            SendRequestViaHttpBody(URI: "media/" + media_id + "/unlike/", method: .post, httpbody: "ig_sig_key_version=4&signed_body=" + sign_body,
                                   success:{
                                    (JSONResponse) -> Void in
                                    print(JSONResponse)
                                    },
    ...
    func login(success:@escaping (JSON) -> Void, failure:@escaping (Error) -> Void) -> Void {
